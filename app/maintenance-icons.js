...
(() => {
  ...
  const ICONS = {
    ...
    chainSlider: '<path d="M4 15c4-4 8-5 16-4"/><path d="M5 18c4-3 8-4 14-3"/><circle cx="6" cy="9" r="2"/><circle cx="18" cy="8" r="2"/>',
  ...
  function decorateDashboard() {
    document.querySelectorAll('.dash-card').forEach((card) => {
      ...
      if (label !== '下次保養' || card.querySelector('.maintenance-icon')) return;
      const key = keyFromName(card.querySelector('strong')?.textContent);
