...
- 依 CB350 週期計算下次里程與日期，並用進度條顯示週期用掉多少
- 保養歷史依「進廠次」分組，同一天同里程的項目收在一張維修單裡
- 花費總覽：累計金額、進廠次數、平均每次、每 1,000 km 成本、依項目與年度統計
- 匯出 JSON 備份
- 雲端同步（需自行部署 `api/sync.js` 與 Redis），設定頁有連線測試按鈕
...
```
今天 里程 12850，換機油 10W-40、清潔潤滑鏈條、檢查煞車皮，費用 950 元
```
...
App 的設定頁有「測試同步連線」按鈕，會直接顯示結果。也可以自己開：
